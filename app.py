...
app.config['SESSION_COOKIE_HTTPONLY'] = True # Ensure it's not accessible via JS
...
# NEW: Debugging endpoint to check session status from frontend
@app.route('/check_session', methods=['GET'])
def check_session():
    if 'username' in session:
        print(f"Backend /check_session: User {session['username']} is in session.")
        return jsonify({"logged_in": True, "username": session['username'], "tier": session.get('tier', 'free')}), 200
    else:
        print("Backend /check_session: No user in session.")
        return jsonify({"logged_in": False}), 200
...
def generate_explanation():
    # NEW: Log session status at the start of this route
    print(f"Backend /generate_explanation: Session at start: {session.get('username')}")

    if 'username' not in session:
        print("Backend /generate_explanation: Unauthorized - username not in session.")
